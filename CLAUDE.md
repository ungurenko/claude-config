...
- **Vercel Serverless + DB пул:** `max: 1` — каждый preview-деплой создаёт свой пул, серия пушей = серия деплоев = N×max соединений. Пуши пачками, не по одному
- **API catch-блоки:** всегда логируй `err.message` + `err.code`, а не просто `"Ошибка сервера"`
- **Env-переменные:** проверяй наличие критичных переменных (`DATABASE_URL`) при инициализации модуля — preview scope может их не включать
