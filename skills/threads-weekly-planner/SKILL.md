...
**CTA:** Выбери случайный вариант из `ctaVariants` в конфиге, подставь ссылку из `ctaLink`. Чередуй варианты CTA — не повторяй подряд. CTA всегда в ПОСЛЕДНЕЙ части поста.

### Алгоритм разбивки на части (multi-part thread)

Лимит одного поста в Threads — `charsPerPart` символов (по умолчанию 500). Длинный контент разбивается на цепочку постов.

**Алгоритм:**
1. Сгенерируй полный текст поста (до 2000 символов, без CTA)
2. Если текст ≤ `charsLastPartText` символов (по умолчанию 420) → **одна часть**: текст + CTA (≤500 итого)
3. Если текст > `charsLastPartText` символов → **multi-part thread**:
   - Разбей на N частей (N ≤ `maxPartsPerThread`, по умолчанию 5)
   - Части 1..N-1: до `charsPerPart` символов каждая, разбивать **по границе предложения** (точка, восклицательный, вопросительный знак)
   - Часть N (последняя): до `charsLastPartText` символов текста + CTA (~80 символов) = ≤`charsPerPart`
   - Если частей получается больше `maxPartsPerThread` — сократи текст
4. Оптимальная длина цепочки: 3-5 частей. Больше 5 — пользователи не дочитывают.

**Вывод:** Покажи все посты пользователю с нумерацией, датой/временем и счётчиком символов.
- Для однокомпонентных постов: как обычно
- Для многокомпонентных постов: покажи каждую часть с разметкой `[1/N]`, `[2/N]`, ... `[N/N]` и счётчиком символов для каждой части
