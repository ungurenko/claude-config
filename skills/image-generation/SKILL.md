...
Параметры: модель X, aspect_ratio Y, ...
Стоимость: ~$0.13 (Gemini) / ~18-24 кредита (nano-banana-pro) / ...

Генерировать? (это стоит денег)
```

Используй `AskUserQuestion`:
```
question: "Генерировать с этими параметрами?"
header: "Confirm"
options:
  - label: "Да, генерируй"
    description: "Запустить генерацию с показанными параметрами"
  - label: "Нет, изменить промпт"
    description: "Хочу поправить промпт или параметры перед генерацией"
```
